...
<script>
	...
	function  toHtml( data){
		...
		alert("성공");
			...
			if(item2.currentGrp > 1){
				str2 += "<div class=\"page_num\"> << </div>";
			}
			for(let i = item2.grpStartPage; i <= item2.grpEndPage ; i++){
				str2 += "<div class=\"page_num\" id=\""+i+"\" onclick=\"currentPageList("+i+"); pageClick("+i+");\">"+i+"</div>";
			}
	...
	function pageClick(num){
		// 요소를 선택
		let element = document.getElementById(num); // 또는 다른 방법으로 요소를 선택

		// 클래스 추가
		element.classList.add("page_click");
	}
